...
std::string MipsGen::gen_text_section(SymbolTable *st, TypeIcgVec &instrs)
{
    ...
    {   
        ...
        // select registers and possibly generate ST, LD
        GetRegRes *res = this->get_reg(i);
        if (res != NULL)
            for (auto ii : res->instrs)
                code += ii->to_string();
...
GetRegRes *MipsGen::get_reg(IcgInstr *i)
{
    if (i->op_code <= OP_NEG)
        return this->get_reg_for_op(i);
    else if (i->op_code == OP_ASGN)
        return this->get_reg_for_copy(i);
    
    return NULL;
}

GetRegRes *MipsGen::get_reg_for_op(IcgInstr *i)
{
    GetRegRes *res = new GetRegRes();
